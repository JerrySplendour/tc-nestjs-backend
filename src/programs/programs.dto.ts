...
export class ProgramDto {
    ...
    @IsOptional() @Type(() => Number) @IsInt() maxParticipants?: number;
    @IsOptional() @IsString() thumbnail?: string;
    @IsOptional() details?: any;
}

export class EnrollDto {
    @Type(() => Number) @IsInt() @Min(1) programId!: number;
}
...
export class OnboardingDto {
    @IsArray() @Type(() => Number) @IsInt({ each: true }) selectedPrograms!: number[];
}
